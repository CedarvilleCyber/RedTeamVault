Default Creds Tool: https://github.com/ihebski/DefaultCreds-cheat-sheet
...
## Password Mutations
We use password mutations to expand the capability of a small (or large) wordlist against a problem. Targeting these attacks is important, because using [[Hashcat]] rules or common mutations against a large wordlist exponentially increases cracking time. Rockyou.txt can easily be grown by 64x using only a small rule, and some are MUCH larger. 
...
|`./username-anarchy -i /path/to/listoffirstandlastnames.txt`|Uses username-anarchy tool in conjunction with a pre-made list of first and last names to generate a list of potential username. |
...
| `hydra -L user.list -P password.list <service>://<ip>`                 | Uses Hydra in conjunction with a user list and password list to attempt to guess a password over the specified service.                                                    |
| `hydra -l username -P password.list <service>://<ip>`                  | Uses Hydra in conjunction with a username and password list to attempt to guess a password over the specified service.                                                     |
| `hydra -L user.list -p password <service>://<ip>`                      | Uses Hydra in conjunction with a user list and password to attempt to guess a password over the specified service.                                                         |
